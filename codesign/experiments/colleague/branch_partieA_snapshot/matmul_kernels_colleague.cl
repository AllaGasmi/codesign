// =============================================================
//  LAB 1 ΓÇô OpenCL Matrix Multiplication Kernels
//  All constants injected via Python f-string before compilation
// =============================================================

// ---------------------------------------------------------
// KERNEL 1 : NAIVE (baseline ΓÇô uncoalesced)
// ---------------------------------------------------------
...
// ---------------------------------------------------------
// KERNEL 2 : COALESCED (reference ΓÇô swap id axes)
// ---------------------------------------------------------
...
// ---------------------------------------------------------
// KERNEL 3 : TILING ΓÇô local (shared) memory
// ---------------------------------------------------------
...
//   Asub stored transposed ΓåÆ no bank conflicts on column reads.
//   Each thread computes WPTM├ùWPTN elements (2-D work).
// ---------------------------------------------------------
...
//   private registers before the inner MAD loop ΓåÆ removes
    ...
    int N)
{
    ...
    // Sortie imm├⌐diate si hors limites
    ...
    for (int tile = 0; tile < N / TSK; tile++) {
        ...
        for (int k = 0; k < TSK; k++) {
            ...
            if (row < N && col < N) {
                ...
                // Acc├¿s correct aux composants d'un float4
